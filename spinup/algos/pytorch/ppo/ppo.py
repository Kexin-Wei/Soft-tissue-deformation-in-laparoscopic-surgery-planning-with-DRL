...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            # save and log
            if t% 100 ==0:
                print(f't:{t} a:{ np.round(a,3)} o:{next_o.round(3).tolist()} r:{env.reward_string}')
        ...
        if proc_id() == 0:
            writer.add_scalar('PPO:Reward/train', logger.log_current_row.get('AverageEpRet', ""), epoch)
        logger.dump_tabular()
...
if __name__ == '__main__':
    ...
    logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed, 'output', True)

    ppo(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        train_pi_iters=args.train_iters,train_v_iters=args.train_iters,
